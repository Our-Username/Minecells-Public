...
if __name__ == "__main__":
	...
	board_gen: PuzzleBoardGenerator = PuzzleBoardGenerator(*args)  # instance of my board generator
	pr = cProfile.Profile()
	pr.enable()
	...
	for r in range(len(board_generated)):
		for c in range(len(board_generated[0])):
			item = "-"
			if (r, c) in revealed_tile_set:
				item = str(board_generated[r][c])
			elif board_generated[r][c] == -3:
				item = str(-3)
			if len(item) == 1:
				item = " " + item
			print(item, end=" ")
