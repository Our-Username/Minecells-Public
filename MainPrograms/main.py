...
class MainProgram:
	def __init__(self, surface: Surface, fps: int = 60) -> None:
		"""
			...
			Initializes:
				...
				- self.board_gen_hub: the class that handles random board generation
				- self.validator: the class which handles validation and account management
				- self.level_manager: the class that handles the levels
		...
		self.FONT: font.Font = TEXTBOX_FONT
		...
		self.board: list[list[int]] = []  #the current board
		self.revealed_tiles: set[TilePosition] | None = set()  #the set of tiles that have been revealed to the user for a puzzle board
		self.public_board: list[list[int]] = []  #the board the user can see
		self.minecount: int = 0  #the remaining unflagged mines
		self.start_minecount: int = 0  #the initial minecount when the user starts the game
		self.board_rows: int = 0  #the number of rows in the board
		self.board_cols: int = 0  #the number of columns in the board
		self.spaces: int = 0  #the number of empty tiles on the board
		self.difficulty: int = 0  #difficulty of puzzle style boards
		self.seed: str = ""  #the seed used for board generation
		self.screen: str = ""  #the name of the current screen the user is looking at
		self.start_active: bool = False  #whether the game is waiting for the user to make the first click during gameplay
		self.gameplay_active: bool = False  #whether the user is in gameplay (playing minesweeper)
		self._run: bool = True  #whether the main event loop should run
		self._ready: bool = True  #if false, skips the current click. Prevents the user from accidentally clicking when moving to a new screen
		self.start_time: float = 0  #the time the user started the current level
		self._shift_active: bool = False  #whether the user has the shift key held down
		self.generator: str = ""  #the current generator in use by the custom generator
		self.offset_directions: list[tuple[int, int]] = []  #the directions tiles look at to decide their number for offset boards
		self.offset_active: bool = False  #whether this is an offset board
		self.tile_surface: Surface | None = None  #the surface to display the tiles on
		self.zoomed_tile_surface: Surface | None = None  #the tile surface used for zooming
		self.tile_surface_dims: tuple[float, float] = (10 * self.board_cols * self.POINT_SIZE, 10 * self.board_rows * self.POINT_SIZE)  #the current dimensions of the tile surface
		self.tile_surface_zoom: float = 1  #the current zoom level of the tile surface
		self.tile_surface_offset: tuple[float, float] = (-1 * 60 * self.POINT_SIZE, -1 * 25 * self.POINT_SIZE)  #the distance from the top left of the tile window to the top left of the screen
		self.alive: bool = False  #whether the user is still alive during gameplay
		self.won: bool = False  #whether the user has won the game
		self.finish_time: float = 0  #the final time the user took to complete the level
		self.valid_minecount: bool = False  #whether the minecount passes the validation checks
		self.valid_rows: bool = False  #whether the row count passes the validation checks
		self.valid_cols: bool = False  #whether the column count passes the validation checks
		self.valid_difficulty: bool = False  #whether the difficulty passes the validation checks
		self.valid_offset: bool = False  #whether the offset passes the validation checks
		self.username: str = ""  #the user's username
		self.password: str = ""  #the user's password
		self.password_confirmed: str = ""  #the user's password confirmation
		self.valid_login: bool = False  #whether the user's login passes the validation checks
		self.keybind_dict: dict[str:int] = self.validator.get_keybinds()  #the keybinds the user uses during gameplay
		self.chording: bool = self.validator.get_options()["chording"]  #whether the chording setting is active
		self.theme: str = "Standard"  #the current theme used by objects
		self.current_level: int = 1  #the current level the user is playing
		self.zoom_animation_count: int = 0  #the number of remaining frames for the zoom animation
		self.zoom_animation_multiplier: float = 0  #the zoom conducted during each animation frame
		self.zoom_animation_centre: Coordinate = (0, 0)  #the centre point for the zoom
		
		# multiprocessing variables
		self.workers: list[mp.Process] = []  #the list of parallel processing workers
		self.task_queue: mp.Queue | None = None  #the task queue for the multiprocessing workers
		self.result_queue: mp.Queue | None = None  #the result queue for the multiprocessing workers
		
		# music variables
		self.music_volume: float = self.validator.get_options()["music"]  #the volume of the music
		self.sfx_volume: float = self.validator.get_options()["sfx"]  #the volume of the sound effects
		self.quieten_active: bool = False  #whether the music is currently being quietened (used during gameplay)
	...
	def start_game(self, start_position: TilePosition) -> None:
		...
		match self.generator.lower():
			case "offset puzzle":
				...
				for row, col in self.revealed_tiles:
					...
					self._object_controller.tile_board[row][col].update()
					self._object_controller.tile_board[row][col].set_value(self.board[row][col])
					self._object_controller.tile_board[row][col].set_text(str(self.board[row][col]), pygame.font.Font(resource_path("MainPrograms/Fonts/mine-sweeper.ttf"), int(self.POINT_SIZE * 4)))
		...
		if "puzzle" not in self.generator.lower():
			...
			self.alive, self.won = resolve_left_click(self.POINT_SIZE, self.public_board, self.board, self._object_controller.tile_board, start_position, self.WIN, dirs, self.colour_dict, self.sfx_volume, False)
	...
	def gameplay_left_click(self, mouse_pos: Coordinate) -> None:
		...
		if self.gameplay_active and self._ready and not self.start_active:
			...
			if tile is not None:
				...
				# dig the tile
				self.alive, self.won = resolve_left_click(self.POINT_SIZE, self.public_board, self.board,
	...
	def tutorial_left_click(self, mouse_pos: Coordinate) -> None:
		...
		if self._ready:
			...
			if tile is not None:
				...
				# dig the tile
				resolve_left_click(self.POINT_SIZE, public_board, private_board, tile_board, tile,
								   self.WIN, dirs, self.colour_dict, self.sfx_volume,
								   tutorial=True)
	...
	def tutorial_right_click(self, mouse_pos: Coordinate) -> None:
		...
		if tile is not None:
			...
			# flag the tile
			resolve_right_click(self.POINT_SIZE, public_board, tile_board, tile,
	...
	def on_win(self) -> None:
		...
		#reduce the score if the user is using chording
		if self.chording:
			multiplier *= min(1 / math.log10(self.start_minecount + 1), 1)
	...
	def gameplay_right_click(self, mouse_pos: Coordinate) -> None:
		...
		if self.gameplay_active:
			...
			if tile is not None:
				# flag the tile
				self.minecount = resolve_right_click(self.POINT_SIZE, self.public_board, self._object_controller.tile_board,
													 tile, self.minecount, self._object_controller.box_dict[("minecount_box", "gameplay")],
	...
	def offset_left_click(self, mouse_pos: Coordinate) -> None:
		"""
			Resolve a left click on the offset menu
		...
		#get the current value for the offset board
		...
		if self.offset_active and self._ready:
			...
			if tile not in [None, (2, 2)]:
				...
				#for each tile in the offset board
				for row in range(5):
					for col in range(5):
						
						#if an inactive tile was clicked, switch it to being active
						if offset_board[row][col] == 1 and (row - 2, col - 2) not in self.offset_directions:
							self.offset_directions.append((row - 2, col - 2))
						#if an active tile was clicked, switch it to being inactive
	...
	def login_keydown(self, key: str) -> None:
		...
		#uppercase the key if shift is being pressed
		if self._shift_active:
			key = key.upper()
	...
	def validate_user(self):
		...
		#if login is valid, return to home screen
		if self.valid_login:
	...
	def create_account(self) -> None:
		...
		#if account is valid, return to the home screen
		if self.valid_login:
			self._object_controller.move_to_home_screen()
	...
	def options_left_click(self, mouse_pos: Coordinate) -> None:
		...
		if self._ready:
			...
			for keybind_key in self._object_controller.keybind_dict.keys():
				...
				#if a keybind box has been clicked, display text to rebind the keybind box
	...
	def close_safely(self) -> None:
		"""
			Close the game safely in the case of an error
		...
		#attempt to close the database properly
		try:
			self.validator.close_connection()
		#if the database never connected properly, or the database has been closed already, ignore it and continue
	...
	def main(self, error_occurred: bool) -> None:
		"""
		...
		self.workers, self.task_queue, self.result_queue = self._object_controller.move_to_first_loading_screen(error_occurred)
		...
		while self._run:
			...
			for event in pygame.event.get():
						...
						if self.screen == "gameplay":
							if 60 * self.POINT_SIZE < click_mouse_x < 140 * self.POINT_SIZE and 25 * self.POINT_SIZE < click_mouse_y < 105 * self.POINT_SIZE:
								if self.keybind_dict["flag"] == -2:
									...
									self.gameplay_left_click((click_mouse_x, click_mouse_y))
						# if the screen is options, rebind the keybind to left click
						elif self.screen == "options":
				...
				# if a key is pressed
					...
					#if the current screen is "create account" or "login"
					elif self.screen == "login" or self.screen == "create_account":
						# if the key is a shift key, set the shift state to true
						if event.key == pygame.K_LSHIFT or event.key == pygame.K_RSHIFT:
							self._shift_active = True
						#if the key is backspace, remove a character from the current text box
						elif event.key == pygame.K_BACKSPACE:
							self.login_backspace()
						# if the key is a letter or number, add a character to the current text box
					...
					#if the screen is options, rebind the key
					elif self.screen == "options":
			...
			# reset the volume
			#get the current volume of the music mixer
			mixer_volume = pygame.mixer.music.get_volume()
			
			#if the current volume of the music mixer is different from the intended volume, or the quieten setting is active
			if mixer_volume != self.music_volume or self.quieten_active:
				
				#if no sound effects are active
				...
				#else if the quieten setting is active, and the music is not quietened
				...
				# get the new volume of the music mixer
				mixer_volume = pygame.mixer.music.get_volume()
				
				#if the difference between the mixer volume and the intended volume is small, set the mixer volume to be the intended volume
...
if __name__ == "__main__":
	...
	main_monitor: Monitor | None = None
	for monitor in get_monitors():
		if monitor.is_primary:
			main_monitor = monitor
			break
	#default to the first monitor in the list
	else:
		main_monitor = get_monitors()[0]
	
	# defines a surface, which is our main screen, and sets it to be fullscreen
	WIN: Surface = pygame.display.set_mode((main_monitor.x, main_monitor.y))
	...
	main_class = MainProgram(WIN, FPS)
	
	#initialize whether there the game has been reopened with an error
	error = False
	
	#until the user exits properly
	while True:
		try:
			#run the main process
			main_class.main(error)
		except ValueError:
			# if there is a value error, restart the program
			main_class.close_safely()
			...
			# if there is an error with the database, restart the program
			main_class.close_safely()
			...
			try:
				# if there is a different error, try and close the game safely
				# no restart as normal exit processes raise errors as well
				main_class.close_safely()
			except:
				break
			else:
