...
class Validation:
	...
	def minecount(self, minecount: int, rows: int, cols: int, old_area: int, spaces: int = 0, generator: str = "Standard") -> bool:
		...
		if minecount == 0:
			...
			#only add a maximum of one of this error
	...
	def rows(self, rows: int, cols: int) -> bool:
		...
		#if the user attempts to generate a large board, indicate that the board may lag
		if rows * cols >= 2500:
	...
	def set_error_text(self, new_text_set: set[str] = None) -> None:
		"""
			Reset the error text to the given value
		
			Inputs:
				- new_text_set: the set of error lines to set the shown error text to
			Outputs:
				- None
		"""
		#Default to empty set if no value is input
		if new_text_set is None:
			self._error_lines = set()
		
		#update with the input values
		else:
	...
	def get_error(self) -> bool:
		"""
			Indicates if there is an error that should prevent the user from continuing
			
			Inputs:
				- None
			Outputs:
				- bool: whether there are severe warnings remaining
		"""
	...
	def get_user_logged_in(self) -> bool:
		...
		#get the list of all the users that have logged in
		...
		if len(logged_in_user_list) > 0:
			...
			#return that there is a user logged in
			return True
		else:
			
			#set the user id to None (no user logged in)
			self.user_id = None
		return False
	
	def get_user_id(self) -> int | None:
		"""
			Getter for the self.user_id attribute
			
			Inputs:
				- None
			Outputs:
				self.user_id: the user id of the logged-in user
	...
	def get_options(self) -> dict[str:bool]:
		...
		#default options if the user is not logged in
		...
		#return the final options dictionary
		return options_dict
	...
	def get_level_times(self) -> dict[str:int]:
		...
		#get the times from all the levels
	...
	def update_win(self, score: int) -> None:
		"""
			Event on a win in the custom gamemode
			
			Inputs:
				- score: the calculated score from the win
			Outputs:
				- None
		...
		#compare with the new score. If higher, update the high score
	...
	def close_connection(self) -> None:
		"""
			Closes the database connection when the user exits the game
