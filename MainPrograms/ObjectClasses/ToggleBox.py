...
class ToggleBox(BoundingBox):
	def __init__(self, surface: Surface, width: float, height: float, point_size: float, border_colour: Colour, border_width: int, name: str, primary_colour: Colour = (0, 255, 0), secondary_colour: Colour = (255, 0, 0)) -> None:
		"""
			...
			Inputs:
				...
				- primary_colour: the default colour of the box
	...
	def initial_draw(self, active: bool = True) -> None:
		"""
			...
			Inputs:
				- active: whether the box is to be active at the start (default True)
			Outputs
				- None
		
		"""
		
		#updates the self._active variable based on the parameter
		self._active = active
		
		#if active, use the primary colour
		if self._active:
			self._draw_with_colour(self._primary_colour)
		#else draw with the secondary colour
		else:
			self._draw_with_colour(self._secondary_colour)
	...
	def get_name(self) -> str:
		"""
			...
			Inputs:
				- None
			Outputs:
				- self._name: the name of the toggle box
	...
	def update_colour(self, *, primary_colour: Colour = None, secondary_colour: Colour = None, border_colour: Colour = None) -> None:
		"""
			...
			Inputs:
				...
				- primary_colour: the new default colour of the box
				- secondary_colour: the new secondary colour of the box
			Outputs:
				- None
		
		"""
		
		#Update the relevant colour if a colour is passed as a parameter
