...
class KeybindBox(BoundingBox):
	def __init__(self, surface: Surface, width: float, height: float, point_size: float, colour: Colour, border_width: int, name: str, background_colour: Colour = (255, 255, 255), default_key: int = -3):
		"""
			...
			Inputs:
				...
				- background_colour: the colour of the background of the keybind box
			...
			Initializes:
				...
				- self._WIN: the main surface which the box is going to be drawn to
				- self._POINT_SIZE: the global point size
				- self.background_colour: the colour of the background of the keybind box
		"""
	...
	def set_key(self, new_key: int) -> None:
		...
		#an input of -1 indicates right click
		if new_key == -1:
			self._text = "Right Click"
		# an input of -2 indicates left click
		elif new_key == -2:
			self._text = "Left Click"
		#else get the name of the key and replace _ with spaces
		else:
	...
	def get_key(self) -> int:
		"""
			...
			Inputs:
				- None
			Outputs:
				- self._key: the integer value of the current key displayed
	...
	def set_text_value(self, text) -> None:
		"""
			Setter for the self._text attribute
	...
	def get_text(self) -> str:
		"""
			Getter for the self._text attribute
