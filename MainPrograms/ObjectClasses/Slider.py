...
class SliderOrb:
	...
	def clear(self) -> None:
		"""
			Clears the circle off the screen by drawing a blank circle on top of it
	...
	def move(self, new_position: float) -> None:
		"""
			Moves the position of the orb and redraws it
			
			Inputs:
				- new_position: the new position of the orb on the screen
			Output:
				- None
		"""
	...
	def check_clicked(self, mouse_pos: Coordinate) -> bool:
		...
		#return result
		return distance_from_centre <= self._radius
